...
[ ! -f "./event-sqs.sample.json" ] && {
    # Just to make sure we are in the right place
    echo "Missing event-sqs.sample.json file, make sure you're in project root folder." >&2
    exit 1
}

APP_NAME=serverless-todo-email-worker
ZIP_FILE_NAME=${APP_NAME}.zip
FILES_TO_ZIP="src/ node_modules/ lambda.js"
SOURCE_FILES="src package.json package-lock.json"
